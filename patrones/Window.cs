...
protected void UpdateGameState(float deltaTime){
    ...
    foreach(var actor in _level.ActorCollection.Values)
    {
...
foreach(string meshid in activeMeshes){
    ...
    if( _level.ActorCollection.TryGetValue("apawn", out pawn))
        {
            pawn.AddComponent(new PawnMovementComponent(_controller,_camera,_level, _controller.Speed, () => {
                Console.WriteLine("¡Has ganado!");
                Close();
            }));
            pawn.AddComponent(new ThirdPersonCameraComponent(_camera,_controller, _controller.CameraDistance, sensibility));
        }
