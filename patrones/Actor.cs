...
public class Actor
{
    ...
    {
        Model=Matrix4.CreateScale(scale)*Matrix4.CreateFromAxisAngle(axisVector,(float) (angle*Math.PI/180.0))*Matrix4.CreateTranslation(positionVector);
    ...
    public void SetCollisionGeometry(Dictionary<string,Asset> AssetCollection)
    {
        if(! AssetCollection.ContainsKey(CollisionMeshId))
                return;
        Mesh CollisionMesh = (Mesh) AssetCollection[CollisionMeshId];
